...
struct Mat44 {
	...
	explicit Mat44(Vec4 const& iBasis3D, Vec4 const& jBasis3D, Vec4 const& kBasis3D, Vec4 const& translation3D);
	explicit Mat44(float const* sixteenValuesBasisMajor);

	static Mat44 const CreateTranslation2D(Vec2 const& translationXY);
	static Mat44 const CreateTranslation3D(Vec3 const& translationXY);
	...
	static Mat44 const CreateLookAtMatrix(Vec3 const& forward);
	...
	Vec3 const RightAppendVectorQuantity3D(Vec3 const& vectorQuantityXYZ);
	...
	void Add(Mat44 const& otherMatrix);
